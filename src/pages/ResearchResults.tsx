...
export default function ResearchResults() {
  ...
  useEffect(() => {
    if (getJobStatus.data) {
      ...
      if (
        getJobStatus.data.status === "completed" &&
        getJobStatus.data.scores
      ) {
      ...
      <ProtectedLayout>
        <div className="text-center py-20">
          <p className="text-slate-500 mb-4">
            কোনো রিসার্চ ফলাফল পাওয়া যায়নি
          </p>
    ...
    try {
      const reportResult = await reportMutation.mutateAsync({
        ...
        scores: scores || {
          totalScore: 0,
          grade: "N/A",
          recommendation: "লাইভ ডাটা অসম্পূর্ণ",
        },
      });
  ...
  const hasScoreData = Boolean(
    scores && Object.keys(scores).some(key => key.endsWith("Score"))
  );

  const scoreItems = hasScoreData
    ? [
        {
          label: "প্রাইজ রেঞ্জ",
          score: scores.priceScore ?? 0,
          icon: DollarSign,
          tooltipText:
            "সুইট স্পট $২০-৩৫। $১৫ এর নিচে ফি বেশি কাটে, $৫০ এর ওপরে রিটার্ন রিস্ক ও যাচাইকরণ বেশি লাগে।",
        },
        {
          label: "সাইজ/ওজন",
          score: scores.sizeWeightScore ?? 0,
          icon: BarChart3,
          tooltipText:
            "স্মল স্ট্যান্ডার্ড সাইজ (<১ পাউন্ড আদর্শ, ৩ পাউন্ড সর্বোচ্চ) সবচেয়ে সস্তা FBA ফি এর জন্য।",
        },
        {
          label: "মার্কেট সাইজ",
          score: scores.marketSizeScore ?? 0,
          icon: TrendingUp,
          tooltipText:
            "BSR ৫০,০০০ এর নিচে থাকলে ভালো মার্কেট ডিমান্ড নির্দেশ করে।",
        },
        {
          label: "রিভিউ ব্যারিয়ার",
          score: scores.reviewBarrierScore ?? 0,
          icon: CheckCircle,
          tooltipText:
            "কম্পিটিটরদের রিভিউ সংখ্যা ১৫০ এর নিচে হওয়া আদর্শ, ৫০০ এর ওপরের রিভিউ অতিক্রম করা কঠিন।",
        },
        {
          label: "ডিফারেন্সিয়েশন",
          score: scores.differentiationScore ?? 0,
          icon: Sparkles,
          tooltipText:
            "নেগেটিভ রিভিউ পড়ে প্রোডাক্টে অন্তত ২-৩টি উন্নয়ন করার সুযোগ থাকা উচিত।",
        },
        {
          label: "সিজনালিটি",
          score: scores.seasonalityScore ?? 0,
          icon: Clock,
          tooltipText:
            "সারা বছর চাহিদা থাকে এমন প্রোডাক্ট নির্বাচন করুন। ৬ সপ্তাহের ট্রেন্ডি প্রোডাক্ট এড়িয়ে চলুন।",
        },
        {
          label: "কমপ্লেক্সিটি",
          score: scores.complexityScore ?? 0,
          icon: Zap,
          tooltipText: "সহজ, ব্যাটারিহীন ও ভঙ্গুর নয় এমন প্রোডাক্ট বেছে নিন।",
        },
        {
          label: "রিটার্ন রেট",
          score: scores.returnRateScore ?? 0,
          icon: AlertTriangle,
          tooltipText:
            "কাঁচ, ইলেকট্রনিক্স ও ক্লোথিং ক্যাটাগরিতে রিটার্ন রেট বেশি থাকে, সাধারণ প্রোডাক্টের রিটার্ন রেট কম।",
        },
        {
          label: "ব্র্যান্ড ডোমিনেন্স",
          score: scores.brandDominanceScore ?? 0,
          icon: BarChart3,
          tooltipText:
            "কোনো একক ব্র্যান্ডের পেজ ১-এ ৪০% এর বেশি দখল থাকা উচিত নয়।",
        },
        {
          label: "ট্রেন্ড",
          score: scores.trendScore ?? 0,
          icon: TrendingUp,
          tooltipText:
            "গত ২৪ মাসের গুগল ট্রেন্ড গ্রাফ সোজা বা ওপরের দিকে থাকা প্রোডাক্টগুলো ভালো।",
        },
        {
          label: "ডিফেন্সিবিলিটি",
          score: scores.defensibilityScore ?? 0,
          icon: Shield,
          tooltipText:
            "প্রোডাক্টের পেটেন্ট, ইউনিক প্যাকেজিং বা কাস্টম বান্ডেল ডিফেন্স তৈরি করে।",
        },
        {
          label: "ম্যানুফ্যাকচারেবিলিটি",
          score: scores.manufacturabilityScore ?? 0,
          icon: Zap,
          tooltipText:
            "Alibaba-তে একাধিক গোল্ড/ভেরিফাইড সাপ্লায়ার থাকা প্রোডাক্ট দ্রুত তৈরি করা যায়।",
        },
        {
          label: "নেট মার্জিন",
          score: scores.marginScore ?? 0,
          icon: DollarSign,
  ...
  const computedTotalScore = scores
    ? Object.values(scores)
        .filter(val => typeof val === "number")
        .reduce((a: any, b: any) => a + b, 0)
    : 0;

  const totalScore = scores?.totalScore ?? computedTotalScore;
    ...
    <ProtectedLayout>
      <div className="space-y-6 max-w-7xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-slate-500">
                {searchConfig?.keyword || searchConfig?.url} (
                {searchConfig?.marketplace || "US"})
              </p>
            </div>
          </div>
          <div className="flex gap-3 flex-wrap">
            <Button
              ...
              disabled={isSaving || !isCompleted || isSaved}
              className="rounded-xl"
            >
              {isSaving ? (
            ...
            <Button
              ...
              disabled={isGeneratingPdf || !isCompleted}
              variant="outline"
            ...
            <CardContent className="p-8">
              <div className="text-center">
                ...
                <p className="text-slate-600 dark:text-slate-300 mb-4">
                  আপনার রিসার্চ জব কিউতে আছে। এটি সম্পন্ন হতে ২-৫ মিনিট সময়
                  লাগতে পারে।
                </p>
            ...
            <CardContent className="p-6">
              <div className="text-center">
                <p className="text-red-600 dark:text-red-400 font-medium">
                  রিসার্চ ব্যর্থ হয়েছে:{" "}
                  {pollError || jobStatus?.error || "Unknown error"}
                </p>
        ...
        {isCompleted && (
          <div
            id="report-content"
            className="grid grid-cols-1 lg:grid-cols-3 gap-6 bg-white dark:bg-slate-900 p-2 sm:p-4 rounded-2xl"
          >
            {/* Left sidebar */}
            <div className="lg:col-span-1 space-y-4">
              ...
              <Card className={`border-0 shadow-xl overflow-hidden ${gc.bg}`}>
                <CardContent className="p-0">
                  <div
                    className={`p-5 ${gc.bg.replace("50", "100")} border-b ${gc.border}`}
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        {gc.icon}
                        <span className={`font-bold ${gc.color}`}>
                          চূড়ান্ত সুপারিশ
                        </span>
                      </div>
                      <Badge
                        className={`${gc.bg} ${gc.color} border ${gc.border} text-lg font-bold`}
                      >
                        {grade}
                      </Badge>
                    </div>
                    <p className={`mt-2 text-sm ${gc.color} font-medium`}>
                      {recommendation}
                    </p>
                  </div>
                  <div className="p-5">
                  ...
                  {fullReportMarkdown ? (
                    ...
                    <Card className="border-0 shadow-lg">
                      <CardContent className="p-8 text-center">
                        ...
                        <p className="text-slate-500 mb-4">
                          AI রিপোর্ট জেনারেট করতে উপরের বাটনে ক্লিক করুন
                        </p>
                        <Button
                          onClick={handleGenerateReport}
